---
title: "Visualizing TMDB ratings"
...
Data was found on [Kaggle.com](https://www.kaggle.com/deepmatrix/tmdb-5000-movie-dataset) and drawn from the [Internet Movie Database](tmdb.com). The code to create the plots can be found in the accompanying GitHub [repo](https://github.com/jstevenr/tmdb-ratings) for this project, as `tmdb.Rmd`.
...
```{r, data cleaning}
all_movies <- read.csv("~/Documents/git-workspace/tmdb-ratings/movie_metadata.csv",
...
all_movies <- subset(all_movies, num_voted_users >= 20000, 
                 select = c(tmdb_score, title_year, movie_title, director_name, gross),                  drop = TRUE)
...
We also want to create a matrix that will keep a record of the summary statistics for the scores of each range of years. 
...
stats[1,1:6] <- round(summary(all_movies$tmdb_score), 2)
stats[1,7] <- nrow(all_movies)

# storing the all-time average
all_avg <- stats[1,4]
...
stats[2,1:6] <- round(summary(movies$tmdb_score), 2)
stats[2,7] <- nrow(movies)

# storing the 'modern' average
modern_avg <- stats[2,4]
stats
```

In the process of dropping films before 1980, the average scores were scarcely affected. This is most likely because of the fact that neither TMDB nor the internet existed since the earliest days of film. Knowing this, rather than look at the average score across the entirety of film we're only going to look at the average between 1980-2016, and compare these films with that metric. 
...
tens <- tens[order(tens$tmdb_score, decreasing = TRUE),]

stats[3,1:6]<- round(summary(tens$tmdb_score), 2)
stats[3,7] <- nrow(tens)
stats
```
...
tens_plot <- ggplot(tens, aes(x = factor(title_year), y = tmdb_score)) +
  geom_boxplot(fill = "#4271AE", colour = "#1F3552") +
  theme(legend.position = "none") +
  theme(axis.text.x = element_text(hjust = 1)) + 
  scale_y_continuous(breaks = seq(0,10,1), name = "TMDB Rating") +
  scale_x_discrete(breaks = seq(2010, 2016, 1), name = "Year") + 
  ggtitle("2010s in film as rated by TMDB users") +
  theme_bw() + stat_boxplot(geom = 'errorbar') + geom_jitter() +
  geom_hline(aes(yintercept = modern_avg, colour = "Average TMDB Rating from 1980-2016"), show.legend = TRUE) +
...
zeroes <- zeroes[order(zeroes$tmdb_score, decreasing = TRUE),]

stats[4,1:6]<- round(summary(zeroes$tmdb_score), 2)
...
zeroes_plot <- ggplot(zeroes, aes(x = factor(title_year), y = tmdb_score)) +
  geom_boxplot(fill = "#4271AE", colour = "#1F3552") +
  theme(axis.text.x = element_text(hjust = 1)) +
  scale_y_continuous(breaks = seq(0,10,1), name = "TMDB Rating") +
  scale_x_discrete(breaks = seq(2000, 2009, 1), name = "Year") + 
  ggtitle("2000s in film as rated by TMDB users") +
  theme_bw() + stat_boxplot(geom = 'errorbar') + geom_jitter() +
  geom_hline(aes(yintercept = modern_avg, colour = "Average TMDB Rating from 1980-2016"), show.legend = TRUE) +
  theme(legend.position = "bottom")
zeroes_plot
...
nineties <- nineties[order(nineties$tmdb_score, decreasing = TRUE),]

stats[5,1:6]<- round(summary(nineties$tmdb_score), 2)
...
nineties_plot <- ggplot(nineties, aes(x = factor(title_year), y = tmdb_score)) +
  geom_boxplot(fill = "#4271AE", colour = "#1F3552") +
  theme(legend.position = "none") +
  theme(axis.text.x = element_text(hjust = 1)) + 
  scale_y_continuous(breaks = seq(0,10,1), name = "TMDB Rating") +
  scale_x_discrete(breaks = seq(1990, 1999, 1), name = "Year") + 
  ggtitle("1990s in film as rated by TMDB users") +
  theme_bw() + stat_boxplot(geom = 'errorbar') + geom_jitter() +
  geom_hline(aes(yintercept = modern_avg, colour = "Average TMDB Rating from 1980-2016"), show.legend = TRUE) +
  theme(legend.position = "bottom")
nineties_plot
...
eighties <- eighties[order(eighties$tmdb_score, decreasing = TRUE),]

stats[6,1:6] <- round(summary(eighties$tmdb_score), 2)
...
Speaking of sequels, the worst films of the 80s were several ill-advised sequels to well-known classics, many of which were done without the involvement of original directors or cast members. Of the films here, only *Howard the Duck* is not a sequel. Notably also is the inclusion of the 5th and 8th *Friday the 13th* films. *Halloween III* is the also only film in the series that does not feature the antagonist Michael Myers. 
...
eighties_plot <- ggplot(eighties, aes(x = factor(title_year), y = tmdb_score)) +
  geom_boxplot(fill = "#4271AE", colour = "#1F3552") +
  theme(legend.position = "none") +
  theme(axis.text.x = element_text(hjust = 1)) + 
  scale_y_continuous(breaks = seq(0,10,1), name = "TMDB Rating") +
  scale_x_discrete(breaks = seq(1980, 1989, 1), name = "Year") + 
  ggtitle("1980s in film as rated by TMDB users") +
  theme_bw() + stat_boxplot(geom = 'errorbar') + geom_jitter() + 
  geom_hline(aes(yintercept = modern_avg, colour = "Average TMDB Rating from 1980-2016"), show.legend = TRUE) +
  theme(legend.position = "bottom")
eighties_plot
...
```{r, echo = FALSE}
movies_plot<- ggplot(movies, aes(x = factor(title_year), y = tmdb_score)) +
  geom_boxplot(fill = "#4271AE", colour = "#1F3552") +
  theme(legend.position = "none") +
  theme(axis.text.x = element_text(hjust = 1)) + 
  scale_y_continuous(breaks = seq(0, 10, 1), name = "TMDB Rating") +
  scale_x_discrete(breaks = seq(1980, 2016, 2), name = "Year") + 
  ggtitle("TMDB Ratings Over the Decades") +
...
It would be interesting to see how TMDB user rating correlates with a film's gross, or how the number of faces on a poster (one of the metrics in the dataset) correlates with the film's rating. Genre may also be something worth scrutinizing (e.g. what genres were most popular in what decades). 
...
----
Data found on [Kaggle.com](https://www.kaggle.com/deepmatrix/tmdb-5000-movie-dataset) and drawn from the [Internet Movie Database](tmdb.com)
