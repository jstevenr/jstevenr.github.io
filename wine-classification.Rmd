...
This was my final project for PSTAT 131, Data Mining, Spring 2017. A version of this project written in Python can be found on my GitHub [here](https://github.com/jstevenr/wine-classification-python/blob/master/wine-quality.ipynb). 
...
knitr::opts_chunk$set(
  ...
  tidy = TRUE, eval = TRUE,
  echo = TRUE, include = TRUE, results = "asis"
)
options(width = 80)
library(tidyverse)
library(class)
...
```{r reading data}
set.seed(10)
white <- read.csv(here::here("data", "winequality-white.csv"),
  sep = ";"
)
...
cormat2 <- round(cor(white2[, 1:9]), 2)
...
white_sc <- white_sc %>%
  mutate(quality = ifelse(quality > 5, yes = "good", no = "bad"))

white_sc$quality <- factor(white_sc$quality, labels = c("bad", "good"))
```
...
# predicting the quality (good vs bad)
tree <- tree(
  formula = quality ~ ., data = train,
  method = "class",
...
# confusion matrix
pruned_conf <- table(pred = pruned_pred, true = test$quality)
pruned_conf
